<!-- contact-partial -->
<div id="content-wrapper" class="contact hidden">

    <!-- contact menu -->
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 contact-menu" onclick="openNav()">
      <span class="open-menu-button">menu</span>
    </div>

    <!-- phone number and email that get copied on click. -->
    <div class="contact-information">
        <div class="dhevak-email">
            <button class="btn email-button" data-clipboard-text="info@dhevak.nl"></button>
        </div>
        <div class="dhevak-phone-number">
            <button class="btn phone-number-button" data-clipboard-text="0621476680"></button>
        </div>
    </div>

    <!-- social media container  -->
    <div class="social-media">
        <a href="https://twitter.com/DhevakArt/media" class="facebook"      target="_blank"></a>
        <a href="https://www.facebook.com/dhevakart/" class="twitter"       target="_blank"></a>
        <a href="https://www.instagram.com/dhevakart/" class="instagram"    target="_blank"></a>
        <a href="https://www.instagram.com/dhevakart/" class="linkedin"     target="_blank"></a>
    </div>

</div>

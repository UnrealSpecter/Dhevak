<video muted class="player projecten pre-intro-left">
    <source id="pre-intro-left-source" type='video/mp4' src="">
</video>
<video muted class="player projecten post-intro-left">
    <source id="post-intro-left-source" type='video/mp4' src="">
</video>
<video muted class="player projecten post-intro-right">
    <source id="post-intro-right-source" type='video/mp4' src="">
</video>
<video muted class="player projecten main">
    <source id="main-source" type='video/mp4' src="">
</video>
<video muted class="player projecten outro-left">
    <source id="outro-left-source type='video/mp4'" src="">
</video>
<video muted class="player projecten outro-right">
    <source id="outro-right-source type='video/mp4'" src="">
</video>
